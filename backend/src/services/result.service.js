...
async function dimensionErrors(items) {
  ...
  const programYears = new Map(programs.map((row) => [row.id, row.year]));
  const specificCollectionPrograms = new Set(
    programs.filter((row) => row.catalog.code === PACTO_CATALOG_CODE).map((row) => row.id),
  );
  ...
  return items.map((item) => {
    ...
    if (Number(item.year) !== programYears.get(item.programId)) {
      return `O resultado deve pertencer ao ciclo ${programYears.get(item.programId)} deste programa`;
    }
